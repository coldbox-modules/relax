...
component singleton {
	...
	function generate(){
		...
		for ( var thisFunction in aSortedNames ) {
			...
			if ( NOT structKeyExists( thisMD, "returntype" ) ) {
				thisMD.returntype = "any";
			}
			if ( NOT structKeyExists( thisMD, "hint" ) ) {
				thisMD.hint = "";
			}
			if ( NOT structKeyExists( thisMD, "access" ) ) {
				thisMD.access = "public";
			}

			// Exclude certain functions
			if (
				listFindNoCase( fncExclude, thisMD.name ) OR
				listFindNoCase( "private,package", thisMD.access )
			) {
				continue;
			}
			...
			if ( arrayLen( thisMD.parameters ) ) {
				...
				for ( var y = 1; y lte arrayLen( thisMD.parameters ); y++ ) {
					if ( NOT structKeyExists( thisMD.parameters[ y ], "required" ) ) {
						thisMD.parameters[ y ].required = false;
					}
					if ( NOT structKeyExists( thisMD.parameters[ y ], "hint" ) ) {
						thisMD.parameters[ y ].hint = "";
					}
					if ( NOT structKeyExists( thisMD.parameters[ y ], "type" ) ) {
						thisMD.parameters[ y ].type = "any";
					}
					if ( NOT structKeyExists( thisMD.parameters[ y ], "default" ) ) {
						thisMD.parameters[ y ][ "default" ] = "---";
					}

...
----------------------------------------------------------------------->
<cfcomponent output="false" hint="The Relax DSL Service" accessors="true" singleton>

	<!--- DI --->
	<cfproperty name="log" 		inject="logbox:logger:{this}">
	<cfproperty name="settings" inject="coldbox:setting:relax">
	<!--- Properties --->
	<cfproperty name="APIDefinitions">
	...
	<cffunction name="listAPIs" output="false" access="public" returntype="any" hint="Get a listing of all the APIs in the resources folder">
    	...
    	<cfdirectory action="list" directory="#variables.settings.APILocationExpanded#" name="results" recurse="false" sort="asc" type="dir">
	...
	<cffunction name="getAPI" output="false" access="public" returntype="any" hint="Get an API definition">
    	...
    	<cfreturn variables.APIDefinitions[ arguments.name ]>
    </cffunction>
    ...
    <cffunction name="loadAPI" output="false" access="public" returntype="any" hint="Load an API into the service and get its representation">
    	...
    	<cfscript>
			...
			if( fileExists( variables.settings.APILocationExpanded & "/#arguments.name#/Relax.cfc" ) ){
				...
				throw(message="The API you are trying to load: #arguments.name# does not exist",detail="Path: #variables.settings.APILocationExpanded#",type="Relax.InvalidDefaultAPI")
			}

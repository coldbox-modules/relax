...
define( [ "Backbone", "models/RelaxerHistory" ], function( Backbone, HistoryModel ){
    ...
    var Relaxer = Backbone.View.extend( {
        ...
        onRelaxerSend : function( e ){
            ...
            if( $( ".advancedSettings", _this.$el ).hasClass( "in" ) ){
                $( ".advancedSettings", _this.$el ).removeClass( "in" );
            }
        ...
        marshallRelaxerRequest: function() {
            ...
            if( $( '[name="httpProxyPort"]', _this.$relaxerForm ).val().length > 0 ){
                request.httpProxyPort = $( '[name="httpProxyPort"]', _this.$relaxerForm ).val();
            }
            // Basic Auth
            ...
            $( ".requestParams", _this.$relaxerForm ).find( ".dynamicField" ).each( function(){
                ...
                request.params[ $paramContainer.find( '[name="parameterName"]' ).val() ] = $paramContainer.find( '[name="parameterValue"]' ).val();
            } );
        ...
        clearHistory : function() {
            ...
            $.post( 
                "#event.buildLink( prc.xehPurgeHistory )#", 
                {}, 
                function( response ){
